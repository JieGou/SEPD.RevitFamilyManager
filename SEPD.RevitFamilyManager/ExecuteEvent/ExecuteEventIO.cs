...
namespace SEPD.RevitFamilyManager
{
    ...
    public class ExecuteEventIO : IExternalEventHandler 
    {
        ...
        public void Execute(UIApplication app)
        {
            ...
            try
            {
                ...
                for (int k = 0; k < familyFilePaths.Count(); k++)
                {
                    ...
                    //产生excel属性表并获得路径
                    XlsFilePath = ExcelHelper.Creating_Excel_Information(gettingFamilyParamenters(rfadoc), familyFilePaths[k]);
                    rfadoc.Close();
                    openFamily.Commit();
        ...
        public DataTable gettingFamilyParamenters(Document rfadoc)
        {
            ...
            openFamilyParaDataTable.Columns.Add("paragroup", typeof(String));
            openFamilyParaDataTable.Columns.Add("paratype", typeof(String));
            ...
            if (rfadoc.IsFamilyDocument)
            {
                ...
                for (int ii = 0; ii < rfadocParasList.Count(); ii++)
                {
                    ...
                    if (rfadocParaName != null)
                    {
                        ...
                        openFamilyParaDataTable.Rows[countNum]["paragroup"] = rfadocParasList[ii].Definition.ParameterGroup.ToString();
                        openFamilyParaDataTable.Rows[countNum]["paratype"] = rfadocParasList[ii].Definition.ParameterType.ToString();
                    }
                    else
                    { continue; }
                    countNum++;
