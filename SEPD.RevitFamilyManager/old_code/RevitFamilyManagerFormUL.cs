...
namespace SEPD.RevitFamilyManager
{
    ...
    public partial class RevitFamilyManagerFormUL : System.Windows.Forms.Form
    {
        //默认链接参数
        #region  默认链接参数
        ...
        ExternalEvent ExternalEventU = null;
        ...
        ////初始化配置文件
        //DataSet dsConnectionInfo = new DataSet();
        //XmlHelper XmlHelper = new XmlHelper();
        //Dictionary<string, string> dConfig = new Dictionary<string, string>();
        //public void configFile()
        //{

        //    dConfig.Add("serverIP", "10.193.217.38");
        //    dConfig.Add("ftpConnectionLocation", "ftp://10.193.217.38/");
        //    dConfig.Add("sqlConnectionLocation", "server=10.193.217.38,1433;uid=sa;Password=SanWei2209;database=DB_Family_Library");
        //    dConfig.Add("ftpUserName", "Administrator");
        //    dConfig.Add("ftpPassword", "LvJW2209");
        //    dConfig.Add("sqlUserName", "sa");
        //    dConfig.Add("sqlPassword", "SanWei2209");
        //    dConfig.Add("sqlDatabase", "DB_Family_Library");
        //    dConfig.Add("xlsLocation", @"C:\ProgramData\Autodesk\Revit\Addins\2016\SepdBuliding\parameter_excel\");
        //    dConfig.Add("downloadLocation", @"C:\RFA_TMP\");

        //    //检测本地config文件是否存在
        //    string defaultConfigFilePath = @"C:\ProgramData\Autodesk\Revit\Addins\2016\SEPD.RevitFamilyManager.config";
        //    if (!File.Exists(defaultConfigFilePath))
        //    {
        //        //在某个文件夹里面创建 .txt/.jpg 等文件，创建不了文件夹，没有path这个文件夹的话就创建不了（会报错）
        //        File.Create(defaultConfigFilePath);
        //        XmlHelper.xmlFileWrite(dsConnectionInfo, defaultConfigFilePath);
        //        //写入预定信息
        //        MessageBox.Show(defaultConfigFilePath+"  创建成功！");
        //    }
        //    else
        //    {
        //        dsConnectionInfo = XmlHelper.xmlFileReader(defaultConfigFilePath);
        ...
        public RevitFamilyManagerFormUL()
        {
            InitializeComponent();
            configFile();
            //ExecuteEventU = new ExecuteEventU();
        ...
        private void btn_openFamilyPath_Click(object sender, EventArgs e)
        {
            ...
            //localPicPath = GetPicture.getFilePic(familyPath);

            //MessageBox.Show(System.Reflection.Assembly.GetExecutingAssembly().Location);

            //ExcPic.familyFilePath = familyPath;
            //eventHandlerPic.Raise();
            //localPicPath = ExcPic.savePath;
            ...
            ExecuteEventU.familyFilePaths = familyPaths ;
            ExecuteEventU.xlsFilePath = Path.GetDirectoryName(familyPath) + "\\";

            ExternalEventU.Raise();

            //string xlsFilePath = @"C:\ProgramData\Autodesk\Revit\Addins\2016\xlsFilePath.txt";

            //if (File.Exists(xlsFilePath))
            //{
            //    StreamReader sr = new StreamReader(xlsFilePath, Encoding.Default);
            //    string line = sr.ReadLine();
            //    txt_openConfigParaPath.Text = line;
            //    sr.Close();
        ...
        private void btn_UploadFamily_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    #region 判断族名和表名问题
                    ////openFamilyName.Replace(".rfa", "");
                    ////originParaName.Replace(".xls","");
                    bool isSameName = true;
                    //if (openFamilyName.Replace(".rfa", "") == originParaName.Replace(".xls", ""))
                    //{
                    //    isSameName = true;
                    //}
                    //else if(txt_openConfigParaPath.Text == @"C:\ProgramData\Autodesk\Revit\Addins\2016\SepdBuliding\parameter_excel\withoutParameter.xls")
                    //{
                    //    isSameName = true;
                    //}
                    //else
                    //{
                    //    isSameName = false;
                    //    MessageBox.Show("保证表名与族名一致,否则无法上传，如无需上传表，则可点击无附表按钮");
                    //}
                    ////if (txt_openConfigParaPath.Text == @"C:\ProgramData\Autodesk\Revit\Addins\2016\SepdBuliding\parameter_excel\withoutParameter.xls")
                    ////{
                    ////    isSameName = true;
                    ////}
                    ////else { }
                    ...
                    else if (DecisionForm.Continue == true && upLoadKey >= 5 && isSameName == true)
                    {
                        ...
                        //MessageBox.Show(txt_openConfigParaPath.Text.ToString());
                        //string dir = Path.GetDirectoryName(txt_openFamilyPath.Text.ToString());
                        //string tmpPath = txt_openConfigParaPath.Text.ToString();
                        //File.Copy(tmpPath, tmpPath.Replace(".xls", "").Replace(openConfigParaName, openFamilyName) + "_" + ReviewLocalFamily.getFileMD5Hash(tmpPath) + ".xls");
        ...
        private void btn_checkpara_Click(object sender, EventArgs e)
        {
            ...
            if (File.Exists(xlsFilePath))
            {
                StreamReader sr = new StreamReader(xlsFilePath, Encoding.GetEncoding("UTF-8"));
                string line = sr.ReadLine();
                txt_openConfigParaPath.Text = line;
                sr.Close();
